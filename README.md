...
Se ha eliminado el comando docker scan, por lo cual se utilizo el nuevo comando docker scout.

Que era docker scan?
Docker Scan era una interfaz de línea de comandos para ejecutar la detección de vulnerabilidades en sus Dockerfiles e imágenes de Docker, la cual se actualizo y se cambio por docker scout el comado por el cual se cambio a hacer este proyecto.
...
Docker Scan permitia analizar imágenes de Docker existentes por nombre o ID de misma manera docker scout.

* You can then use `docker scout DOCKER_IMAGE`:
```console
docker scout cves scout
    i New version 1.15.1 available (installed version is 1.14.0) at https://github.com/docker/scout-cli
          v SBOM of image already cached, 62 packages indexed
    x Detected 7 vulnerable packages with a total of 28 vulnerabilities
...
────────────────────┼─────────────────────────────────────
  Target            │  scout:latest 
    ...
    vulnerabilities │    3C    16H    19M     1L     2?
    size            │ 24 MB
    packages        │ 62
